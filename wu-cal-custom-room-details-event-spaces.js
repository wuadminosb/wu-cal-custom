(() => {
  ...
  const STYLE_ID = 'wu-event-space-details-style';
  const ROOT_CLASS = 'wu-event-details-active';
  ...
  function findRoom(title) {
    return (window.wuEventSpaceRooms || []).find(room =>
      room.match.some(token =>
        norm(title).includes(norm(token))
      )
    ) || null;
  }
  ...
  function apply() {
    ...
    if (
      active?.room === room.key &&
      active.root === root &&
      active.row?.isConnected &&
      (
        !active.official?.children.length ||
        active.official?.isConnected
      ) &&
    ...
    room.tech.forEach(([titleText, html, groupName]) => {
      ...
      details.innerHTML =
        '<summary>' +
        '<span></span>' +
        '<span class="wu-evt-icon" aria-hidden="true">+</span>' +
        '</summary>' +
      ...
      details.addEventListener('toggle', () => {
        details.querySelector('.wu-evt-icon').textContent =
          details.open ? '−' : '+';
      });
    ...
    settingsSection.innerHTML =
      '<h2>Settings Standard</h2>' +
      '<div class="wu-evt-grid"></div>';
    ...
    room.settings.forEach(([titleText, imageUrl, pdfUrl]) => {
      ...
      card.innerHTML =
        '<a class="wu-evt-img" target="_blank" rel="noopener noreferrer">' +
        '<img loading="lazy">' +
        '</a>' +
        '<div class="wu-evt-body">' +
        '<h3></h3>' +
        '<a class="wu-evt-pdf" target="_blank" rel="noopener noreferrer">' +
        '↓ PDF öffnen' +
        '</a>' +
        '</div>';
      ...
      image.src = imageUrl;
      image.alt = 'Stellplan ' + titleText;

      card.querySelectorAll('a').forEach(link => {
        link.href = pdfUrl;
        link.setAttribute(
          'aria-label',
          'Stellplan ' + titleText + ' als PDF öffnen'
        );
      });
    ...
    if (main && mainImage && thumbs.length) {
      ...
      const thumbClick = event => {
        ...
        const thumbIndex = thumbs.indexOf(button);
  ...
  window[GLOBAL] = {
    ...
    cleanup,
    observer,
    rooms: () => window.wuEventSpaceRooms || []
  };
